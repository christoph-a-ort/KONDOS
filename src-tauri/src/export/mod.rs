...
pub fn write_export<W: Write>(
    ...
    let flags = meta_flags_from_root(&result.root);
    match format {
        ExportFormat::Txt => txt::write_txt(writer, result, flags),
        ExportFormat::Csv => csv::write_csv(writer, result, flags),
        ExportFormat::Json => json::write_json(writer, result, flags),
    }
}

pub fn render_export(format: ExportFormat, result: &ScanResult) -> Result<String, io::Error> {
    let mut buffer = Vec::new();
    write_export(&mut buffer, format, result)?;
